...
func validateRequiredFields(issues *[]domainvalidation.Issue, entity *model.CheckedEntity, frontmatter map[string]any, typeSpec model.SchemaEntityType) {
	for _, rule := range typeSpec.RequiredFields {
		...
		if rule.HasValue && !rule.ValueIsExpression && !support.LiteralEqual(value, rule.Value) {
			addIssue(issues, entity, domainvalidation.Issue{
...
func validateRequiredSections(issues *[]domainvalidation.Issue, entity *model.CheckedEntity, sections map[string]string, duplicateLabels []string, typeSpec model.SchemaEntityType) {
	...
	for _, requiredLabel := range typeSpec.RequiredSections {
		if _, ok := sections[requiredLabel]; ok {
			continue
		}
		addIssue(issues, entity, domainvalidation.Issue{
			...
			Level:       domainvalidation.LevelError,
			Class:       "InstanceError",
			Message:     fmt.Sprintf("required section '%s' is missing", requiredLabel),
			StandardRef: "13.2",
			Field:       fmt.Sprintf("content.sections.%s", requiredLabel),
		})
	}
}
