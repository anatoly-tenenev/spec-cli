...
func ParseType(
	...
	allowSetPaths := map[string]model.WritePathSpec{}
	...
	for _, fieldName := range metaOrder {
		...
		path := "meta." + fieldName
		allowSetPaths[path] = model.WritePathSpec{Kind: model.WritePathMeta, FieldName: fieldName}
		allowUnsetPaths[path] = model.WritePathSpec{Kind: model.WritePathMeta, FieldName: fieldName}
	}
	for _, sectionName := range sectionOrder {
		path := "content.sections." + sectionName
		allowSetPaths[path] = model.WritePathSpec{Kind: model.WritePathSection, FieldName: sectionName}
		allowUnsetPaths[path] = model.WritePathSpec{Kind: model.WritePathSection, FieldName: sectionName}
	...
	return model.EntityTypeSpec{
		...
		AllowSetPaths:     allowSetPaths,
		AllowUnsetPaths:   allowUnsetPaths,
