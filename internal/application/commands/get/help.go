...
func HelpSpec() helpmodel.CommandSpec {
	return helpmodel.CommandSpec{
		Name:    "get",
		Summary: "read one entity through a schema-derived read model",
		Syntaxes: []string{
			"spec-cli get [options] --id <entity_id>",
		},
		OperationModel: []string{
			"get reads one entity through a schema-derived single-entity read model.",
			"--id selects the target entity.",
			"--select projects fields from the single-entity read model.",
		},
		DetailSections: []helpmodel.DetailSectionSpec{
			...
			},
			{
				Title: "Defaults",
				Lines: []string{
					"if --select is omitted, default projection:",
					"  - type",
					"  - id",
					"  - slug",
					"  - meta",
		...
		Options: []helpmodel.OptionSpec{
			...
			},
			{
				...
				Repeatable:       true,
				SchemaDerived:    true,
				SchemaDerivation: "selectable paths from the single-entity read model",
				Description:      "Select a path from the single-entity read model.",
			},
		},
		Rules: []string{
