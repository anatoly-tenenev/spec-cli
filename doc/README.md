...
- [CODEBASE_INDEX.md](./CODEBASE_INDEX.md) - a compact codebase map (agent map) in the `entrypoint + responsibilities + subpackages` format for each layer/package, including the current state of `help`/`add`/`update`/`delete`/`version`, the canonical `help` path contract (`ResolvedPath`), the degraded-schema recovery contract in `help`, the pre-dispatch contract of global CLI options and the integration runner (including data-first groups `cases/help/10_general` without a duplicate default case, `cases/help/15_schema_recovery`, `cases/help/20_errors`, optional `workspace.in` for help cases, `workspace.out` for mutating commands, and the `workspace.in/.keep` marker for empty input workspaces), current `query` namespace coverage for scalar `entity_ref` (`meta.<ref_field>` forbidden in `--select`/`--where-json`, `refs.<field>` required), plus current `validate` coverage for scalar/array `entity_ref`, `schema.items.refTypes` constraints, blank array `entity_ref` handling, and dedicated unit-test subpackages.
