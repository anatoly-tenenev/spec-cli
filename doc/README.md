...
- [002-integration/INTEGRATION_CASES_LAYOUT_RU.md](./002-integration/INTEGRATION_CASES_LAYOUT_RU.md) — data-first и black-box контрактная структура интеграционных кейсов, layout `tests/integration/cases`, контракт содержимого кейса (`case.json`, `spec.schema.yaml`, `workspace.in/out`, `response.json`), включая двухуровневую группировку `validate/<group>/<case>` и соглашение для `validate`-кейсов `NNNN_ok_*` / `NNNN_err_*`.
- [002-integration/VALIDATE_STANDARD_COVERAGE_BACKLOG_RU.md](./002-integration/VALIDATE_STANDARD_COVERAGE_BACKLOG_RU.md) — актуальный backlog покрытия `validate` по локальному стандарту: базовые шаблоны ожидаемых ответов, срез фактического покрытия на `2026-03-09`, список уже закрытых пунктов относительно ревизии `2026-03-07` и открытые P0/P1 задачи с неколлидирующими `case-id` для дальнейшего расширения интеграционных кейсов.
