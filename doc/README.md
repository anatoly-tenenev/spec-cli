...
- [002-integration/VALIDATE_STANDARD_COVERAGE_BACKLOG_RU.md](./002-integration/VALIDATE_STANDARD_COVERAGE_BACKLOG_RU.md) — чеклист недостающих интеграционных кейсов `validate` для покрытия локального стандарта (`schema/workspace/response` для каждого `group/case-id`, с приоритетами P0/P1), включая актуальные ожидания SchemaError для strict `path_pattern` условий с потенциально отсутствующими операндами, минимальный `workspace.in/.keep` для schema-level сценариев и дополнительные контрактные кейсы для NDJSON/fail-fast/coverage.
