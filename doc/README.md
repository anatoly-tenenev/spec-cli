...
- [SCHEMA_COMPILER_MIGRATION_PLAN.md](./SCHEMA_COMPILER_MIGRATION_PLAN.md) - phased migration plan from command-local schema loaders to the unified schema compiler, including `schema check`, response-contract rollout, command migration order (`validate`, `query/get`, `add/update`, `delete`), testing expectations, cleanup criteria for legacy schema packages, and an explicit Phase-2 convergence checkpoint to keep `schema check` aligned with current strict schema validity rules and the zero-issues response contract.
